...
Page({
  ...
  onLoad: function (options) {
    ...
    goStartHandler2:function(event){
        ...
        if (!this.data.hastoken){
          wx.showModal({
            ...
            success: function (res) {
              if (res.confirm) {
                wx.reLaunch({
                  url: '/pages/peotry/index',//'/pages/peotry/index'
                });
              } else if (res.cancel) {
                console.log('用户点击取消')
              }
  ...
  goStartHandler:function(event){
      ...
      if (!this.data.hastoken){
        wx.showModal({
          ...
          success: function (res) {
            if (res.confirm) {
              wx.reLaunch({
                url: '../index/index',//'../user/bindphone/index'
              });
            } else if (res.cancel) {
              console.log('用户点击取消')
            }
    ...
    //wx.redirectTo({ url: "../Bird/index?num=" + linkNum});
  },
  goStartHandler1:function(event){
    let linkNum = event.currentTarget.dataset.linknum;
      ...
      if (!this.data.hastoken){
        wx.showModal({
          ...
          success: function (res) {
            if (res.confirm) {
              wx.reLaunch({
                url: '../Bird/index',//'../user/bindphone/index'
              });
            } else if (res.cancel) {
              console.log('用户点击取消')
            }
